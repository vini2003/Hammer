...
import dev.vini2003.hammer.client.util.InstanceUtils.CLIENT
...
object DrawingUtils {
	...
	fun drawQuad(matrices: MatrixStack, provider: VertexConsumerProvider, layer: RenderLayer, x: Float, y: Float, z: Float, sX: Float, sY: Float, light: Int, color: Color) {
		...
		consumer.vertex(matrices.peek().positionMatrix, x, y, z).color(color.r, color.g, color.b, color.a).light(light).next()
		consumer.vertex(matrices.peek().positionMatrix, x, y + sY, z).color(color.r, color.g, color.b, color.a).light(light).next()
		consumer.vertex(matrices.peek().positionMatrix, x + sX, y + sY, z).color(color.r, color.g, color.b, color.a).light(light).next()
		consumer.vertex(matrices.peek().positionMatrix, x + sX, y, z).color(color.r, color.g, color.b, color.a).light(light).next()
	}
	...
	fun drawGradientQuad(matrices: MatrixStack, provider: VertexConsumerProvider, layer: RenderLayer, startX: Float, startY: Float, endX: Float, endY: Float, z: Float, uS: Float, vS: Float, uE: Float, vE: Float, light: Int, colorStart: Color, colorEnd: Color, textured: Boolean) {
		...
		consumer.vertex(matrices.peek().positionMatrix, endX, startY, z + 201).color(colorStart.r, colorStart.g, colorStart.b, colorStart.a).texture(uS, vS).light(light).normal(matrices.peek().normalMatrix, 0f, 1f, 0f).next()
		consumer.vertex(matrices.peek().positionMatrix, startX, startY, z + 201).color(colorStart.r, colorStart.g, colorStart.b, colorStart.a).texture(uS, vE).light(light).normal(matrices.peek().normalMatrix, 0f, 1f, 0f).next()
		consumer.vertex(matrices.peek().positionMatrix, startX, endY, z + 201).color(colorEnd.r, colorEnd.g, colorEnd.b, colorEnd.a).texture(uE, vS).light(light).normal(matrices.peek().normalMatrix, 0f, 1f, 0f).next()
		consumer.vertex(matrices.peek().positionMatrix, endX, endY, z + 201).color(colorEnd.r, colorEnd.g, colorEnd.b, colorEnd.a).texture(uE, vE).light(light).normal(matrices.peek().normalMatrix, 0f, 1f, 0f).next()
	}
	...
	fun drawTexturedQuad(matrices: MatrixStack, provider: VertexConsumerProvider, layer: RenderLayer, x: Float, y: Float, sX: Float, sY: Float, texture: Identifier?) {
		drawTexturedQuad(matrices, provider, layer, x, y, 0F, sX, sY, 0F, 0F, 1F, 1F, 0x00F000F0, Color.WHITE, texture)
	}
	...
	fun drawTexturedQuad(matrices: MatrixStack, provider: VertexConsumerProvider, layer: RenderLayer, x: Float, y: Float, z: Float, sX: Float, sY: Float, texture: Identifier?) {
		drawTexturedQuad(matrices, provider, layer, x, y, z, sX, sY, 0F, 0F, 1F, 1F, 0x00F000F0, Color.WHITE, texture)
	}
	...
	fun drawTexturedQuad(matrices: MatrixStack, provider: VertexConsumerProvider, layer: RenderLayer, x: Float, y: Float, z: Float, sX: Float, sY: Float, u0: Float, v0: Float, u1: Float, v1: Float, light: Int, color: Color, texture: Identifier?) {
		...
		consumer.vertex(matrices.peek().positionMatrix, x, y + sY, z).color(color.r, color.g, color.b, color.a).texture(u0, v1).overlay(DEFAULT_OVERLAY).light(light).normal(matrices.peek().normalMatrix, 0F, 0F, 0F).next()
		consumer.vertex(matrices.peek().positionMatrix, x + sX, y + sY, z).color(color.r, color.g, color.b, color.a).texture(u1, v1).overlay(DEFAULT_OVERLAY).light(light).normal(matrices.peek().normalMatrix, 0F, 0F, 0F).next()
		consumer.vertex(matrices.peek().positionMatrix, x + sX, y, z).color(color.r, color.g, color.b, color.a).texture(u1, v0).overlay(DEFAULT_OVERLAY).light(light).normal(matrices.peek().normalMatrix, 0F, 0F, 0F).next()
		consumer.vertex(matrices.peek().positionMatrix, x, y, z).color(color.r, color.g, color.b, color.a).texture(u0, v0).overlay(DEFAULT_OVERLAY).light(light).normal(matrices.peek().normalMatrix, 0F, 0F, 0F).next()
	}
