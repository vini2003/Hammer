package dev.vini2003.hammer.`interface`.client.screen

import dev.vini2003.hammer.`interface`.common.packet.sync.SyncScreenHandlerPacket
...
open class BaseHandledScreen<T : BaseScreenHandler>(handler: BaseScreenHandler, inventory: PlayerInventory, title: Text) : HandledScreen<T>(handler as T, inventory, title) {
	override fun init() {
		...
		ClientPlayNetworking.send(HIFNetworking.SYNC_SCREEN_HANDLER, buf)
	}
	...
	override fun render(matrices: MatrixStack, mouseX: Int, mouseY: Int, delta: Float) {
		...
		val provider = InstanceUtils.CLIENT.bufferBuilders.effectVertexConsumers
