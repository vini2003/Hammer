...
import dev.vini2003.hammer.client.util.Instances.client
...
object Drawings {
	...
	fun drawTexturedQuad(matrices: MatrixStack, provider: VertexConsumerProvider, layer: RenderLayer, x: Float, y: Float, z: Float, sX: Float, sY: Float, u0: Float, v0: Float, u1: Float, v1: Float, light: Int, color: Color, texture: Identifier?) {
		...
		consumer.vertex(matrices.peek().model, x, y + sY, z).color(color.r, color.g, color.b, color.a).texture(u0, v1).overlay(defaultOverlay).light(light).normal(matrices.peek().normal, 0F, 0F, 0F).next()
		consumer.vertex(matrices.peek().model, x + sX, y + sY, z).color(color.r, color.g, color.b, color.a).texture(u1, v1).overlay(defaultOverlay).light(light).normal(matrices.peek().normal, 0F, 0F, 0F).next()
		consumer.vertex(matrices.peek().model, x + sX, y, z).color(color.r, color.g, color.b, color.a).texture(u1, v0).overlay(defaultOverlay).light(light).normal(matrices.peek().normal, 0F, 0F, 0F).next()
		consumer.vertex(matrices.peek().model, x, y, z).color(color.r, color.g, color.b, color.a).texture(u0, v0).overlay(defaultOverlay).light(light).normal(matrices.peek().normal, 0F, 0F, 0F).next()
	}
	...
	@JvmStatic
	val defaultOverlay = OverlayTexture.DEFAULT_UV
	
	@JvmStatic
	val defaultLight = 0x00f000f0
}